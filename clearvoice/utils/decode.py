...
import librosa
import torchaudio
from clearvoice.utils.misc import (
    power_compress,
    power_uncompress,
    stft,
    istft,
        ...
        while current_idx + window <= t:
            ...
            for spk in range(args.num_spks):
                ...
                if current_idx == 0:
                    ...
                    outputs[
                        spk,
                        current_idx + give_up_length : current_idx
                        + window
                        - give_up_length,
...
def _decode_one_audio_mossformergan_se_16k(model, device, inputs, norm_factor, args):
    ...
    padded_len = int(
        nframe * args.win_inc
    )  # Calculate the padded length to fit the model
    ...
    pred_real, pred_imag = (
        out_list[0].permute(0, 1, 3, 2),
        out_list[1].permute(0, 1, 3, 2),
    )
        ...
        if online_decoding:
            ...
            while current_idx + window <= t:
                ...
                if current_idx == 0:
                    ...
                    generator_output_segment = generator_output_segment[
                        -window:
                    ]  # Get the latest window of output
                    outputs[
                        current_idx + give_up_length : current_idx
                        + window
                        - give_up_length
                    ] = generator_output_segment[
                        give_up_length : -give_up_length + offset
                    ]
