    ...
    def decode(self):
        ...
        for i in range(len(self.data["audio"])):
            output_audio = decode_one_audio(
                self.model, self.device, self.data["audio"][i], self.args
            )
        ...
        if self.args.task == "target_speaker_extraction":
            ...
            with torch.no_grad():
                for idx in tqdm(range(num_samples)):  # Loop over all audio samples
                    ...
                    input_audio, wav_id, input_len, scalars, audio_info = data_reader[
                        idx
                    ]
                        ...
                        if isinstance(output_audios, list):
                            ...
                            for spk in range(self.args.num_spks):
                                output_file = os.path.join(
                                    ...
                                    wav_id.replace(
                                        "." + self.data["ext"],
                                        f"_s{spk + 1}." + self.data["ext"],
                                    ),
                                )
                                self.write_audio(
                                    output_file, key=None, spk=spk, audio=output_audios
                                )
                        else:
                            # Single-speaker or standard output
                            output_file = os.path.join(output_wave_dir, wav_id)
                            self.write_audio(
                                output_file, key=None, spk=None, audio=output_audios
                            )
                    else:
    ...
    def write_audio(self, output_path, key=None, spk=None, audio=None):
        ...
        if self.data["sample_rate"] != self.args.sampling_rate:
            if self.data["channels"] == 2:
                left_channel = librosa.resample(
                    result_[0, :],
                    orig_sr=self.args.sampling_rate,
                    target_sr=self.data["sample_rate"],
                )
        ...
        audio_format = (
            "ipod" if self.data["ext"] in ["m4a", "aac"] else self.data["ext"]
        )
                ...
                if isinstance(self.result[key], list):  # For multi-speaker outputs
                    for spk in range(self.args.num_spks):
                        output_file = output_path.replace(
                            "." + self.data["ext"], f"_s{spk + 1}." + self.data["ext"]
                        )
                        self.write_audio(output_file, key, spk)
                else:
                    self.write_audio(output_path, key)
