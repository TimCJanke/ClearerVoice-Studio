...
class ResBlock1(torch.nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.convs1_activates = nn.ModuleList(
            [Snake1d(channels), Snake1d(channels), Snake1d(channels)]
        )
    ...
    def forward(self, x):
        for c1, c2, act1, act2 in zip(
            self.convs1, self.convs2, self.convs1_activates, self.convs2_activates
        ):
...
class ResBlock2(torch.nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3)):
        ...
        self.convs_activates = nn.ModuleList([Snake1d(channels), Snake1d(channels)])
...
class Mossformer(nn.Module):
    def __init__(self):
        ...
        self.mossformer = MossFormer_MaskNet(
            in_channels=80, out_channels=512, out_channels_final=80
        )
